...
docker run --rm \
    --mount source="${SCRIPT_DIR}",target=/hpcc-dev/wasmtests,type=bind,consistency=cached \
...
echo "docker run -it --mount source=\"${SCRIPT_DIR}\",target=/hpcc-dev/wasmtests,type=bind,consistency=cached wasm32-wasi:latest bash"
