#include "sample_wamr.hpp" // Includes wamr.hpp, sample.hpp and all helpers
#include <algorithm>
#include <cfloat>
#include <filesystem>
#include <fstream>
...
int main(int argc, char **argv)
{
    ...
    // The following functions are world-level exports, not in imported interfaces,
    // so they don't have generated typedefs - using explicit types
    std::cout << "\n--- Variant Functions ---" << std::endl;
    auto variant_func = guest_function<variant_t<bool_t, uint32_t>(variant_t<bool_t, uint32_t>)>(
    ...
    auto option_func = guest_function<option_t<uint32_t>(option_t<uint32_t>)>(
    ...
    auto void_func_guest = guest_function<void()>(module_inst, exec_env, liftLowerContext, "void-func");
    void_func_guest();

    std::cout << "\n--- Result Functions ---" << std::endl;
    auto ok_func = guest_function<result_t<uint32_t, string_t>(uint32_t, uint32_t)>(
    ...
    auto err_func = guest_function<result_t<uint32_t, string_t>(uint32_t, uint32_t)>(
