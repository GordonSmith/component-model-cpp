              ...
              wasm_extern_vec_t *out)
{
    ...
    for (unsigned i = 0; i < importtypes.num_elems; i++)
    {
        ...
        if (strncmp(module_name->data, "example:sample/tuples", strlen("example:sample/tuples")) == 0 && strncmp(field_name->data, "reverse", strlen("reverse")) == 0)
        {
            wasm_functype_t *log_type = wasm_functype_new_2_2(
...
int main()
{
    ...
    printf("> Load .aot\n");
#else
    FILE *file = fopen("build/samples/wasm/src/wasm-build/sample.wasm", "rb");
    printf("> Load .wasm\n");
#endif
    if (!file)
    {
